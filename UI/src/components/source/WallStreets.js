...
class WallStreets extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      articles: [],
  ...
  componentDidMount() {
    this.newsService.getArticlesBySource("wsj.com").then((data) => {
      this.setState({
  ...
  handlePageChange = pageNumber => {
    ...
    this.newsService.getArticlesBySource("wsj.com", pageNumber).then((data) => {
      this.setState({
        articles: data.articles,
        totalResults: data.totalResults,
      });
