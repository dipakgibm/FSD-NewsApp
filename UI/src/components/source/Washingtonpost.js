...
class Washingtonpost extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      articles: [],
  ...
  componentDidMount() {
    this.newsService.getArticlesBySource("washingtonpost.com").then((data) => {
      this.setState({
  ...
  handlePageChange = pageNumber => {
    ...
    this.newsService.getArticlesBySource("washingtonpost.com", pageNumber).then((data) => {
      this.setState({
        articles: data.articles,
        totalResults: data.totalResults,
      });
